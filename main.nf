...
def helpMessage() {
    ...
    Input/output options:
        ...
        For ALL vs ALL Analysis:
            --inpath             Path to input data directory containing FastA and/or Genbank files. Recognized extensions are: {faa,gb,gbk,gbf,gbff}{'',gz}.
        
        For QUERY vs REFERENCE Analysis:
            --query              Path to query input data file that is FastA or Genbank. Recognized extensions are: {faa,gb,gbk,gbf,gbff}{'',gz}.
            --refdir             Path to input data directory containing FastA and/or Genbank files. Recognized extensions are: {faa,gb,gbk,gbf,gbff}{'',gz}.
    
    Analysis options:
      --aligner            Specify aligner to use when aligning proteins. Options are: blast, diamond. Default is set to blast.
...
include { ALIGN_BLAST } from "./subworkflows/local/aligner_blast.nf"
include { ALIGN_DIAMOND } from "./subworkflows/local/aligner_diamond.nf"
...
workflow {
    ...
    pairs_ch = pairs_ch.mix(GENERATE_PAIRS.out.pairs).splitCsv(header:false, sep:'\t').map{row-> tuple(row[0], row[1])}.combine(INFILE_HANDLING.out.prot)

    // PROCESS: Perform AAI on each pair
    if (params.aligner == 'diamond') {
        ...
        // Collect version info
        ch_versions = ch_versions.mix(ALIGN_DIAMOND.out.versions)

        // Collect all AAI stats.tab files and concatenate into one
        aai_stats_ch = aai_stats_ch.mix(ALIGN_DIAMOND.out.stats).collect()
    } else {
        ALIGN_BLAST (
            pairs_ch
        )
        ...
        // Collect version info
        ch_versions = ch_versions.mix(ALIGN_BLAST.out.versions)
